...
**A:** `galay-rpc` 通过 `find_package(galay-kernel CONFIG REQUIRED)` 查找依赖。若你按 README 的并排 clone 方式组织仓库，请先为 `galay-kernel` 生成 package config：
...
本仓库现在只走标准 `find_package(galay-kernel CONFIG REQUIRED)`；如果依赖不在默认安装前缀，可显式指定：
